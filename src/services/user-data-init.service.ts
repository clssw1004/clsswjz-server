...
export class UserDataInitService {
    ...
    async initializeUserData(userId: string): Promise<void> {
        await this.accountFundRepository.manager.transaction(async (transactionalEntityManager) => {
            // 1. 创建默认资金账户
            const defaultFund = await this.initializeDefaultFund(transactionalEntityManager, userId);

            // 2. 创建默认账本
            const defaultBook = await this.initializeDefaultBook(transactionalEntityManager, userId);
